<template>
  <div class="ev-calendar-wrapper">
    <template v-for="calendarType in calendarList" :key="calendarType">
      <div class="ev-calendar-date-area">
        <div class="ev-calendar-header">
          <div class="move-month-arrow" @click="clickPrevNextBtn(calendarType, 'prev')">
            <i class="ev-icon-s-arrow-left move-month-arrow-icon" />
          </div>
          <div class="ev-calendar-year-month-wrapper">
            <span v-if="selectedListType[calendarType] === 'year'" class="ev-calendar-year-range">
              {{
            ...
            <template v-else>
              <span class="ev-calendar-year" @click="clickYearMonthBtn(calendarType, 'year')">
          ...
          <div class="move-month-arrow" @click="clickPrevNextBtn(calendarType, 'next')">
        ...
        <div class="ev-calendar-body">
          ...
          >
            <thead>
              <tr>
                <th v-for="dayOfTheWeek in dayOfTheWeekList" :key="dayOfTheWeek">
                  {{ dayOfTheWeek }}
                </th>
              </tr>
            </thead>
            <tbody @wheel.prevent="wheelMonth(calendarType, $event)">
              <tr v-for="weekInfo in calendarTableInfo[calendarType]" :key="weekInfo">
                <td
          ...
          >
            <tbody @wheel.prevent="wheelMonth(calendarType, $event)">
              <tr
                v-for="rowInfo in selectedListType[calendarType] === 'month'
          ...
          >
            <ev-button @click="clickYearMonthBtn(calendarType, 'date')"> Done </ev-button>
          </div>
        </div>
      </div>

      <div v-if="['dateTime', 'dateTimeRange'].includes(mode)" class="ev-calendar-time-area">
        <div
          v-if="selectedListType[calendarType] !== 'date'"
        ...
        <div class="ev-calendar-time-center">
          <template v-for="timeType in ['hour', 'min', 'sec']" :key="`${timeType}_table`">
            <table class="ev-calendar-time-table">
              <tbody @wheel.prevent="wheelTime(calendarType, timeType, $event)">
                <tr v-for="i in 3" :key="`${timeType}_${i}_tr`">
                  <td
                    v-for="j in 4"
                    ...
                    :class="[
                      ...
                      {
                        disabled:
                          preventTimeEventType[calendarType][timeType] ||
        ...
        <div class="ev-calendar-time-side">
          <template v-for="hmsType in ['hour', 'min', 'sec']" :key="`${hmsType}_btn_area`">
            <div
