...
- **대화형 API 문서 (/api-docs)**: 문서 사이트의 메인 화면은 `/api-docs`(`docs/views/apiDocs/`)이고 기존 컴포넌트 페이지 라우트는 여기로 리다이렉트된다. 이관된 컴포넌트의 API SSOT 는 `docs/views/apiDocs/data/<key>.json`(현재 lineChart, barChart, scatterChart, pieChart, heatMap, button — `data/index.js` 에 등록)이며, 미이관 컴포넌트는 `api/<이름>.md` 폴백 뷰로 렌더된다. JSON 은 `npm run docs:validate`(`scripts/validate-api-docs.js`) 스키마 검증을 통과해야 하고 lint-staged 훅·GitHub Actions 게이트가 이를 강제한다. 스키마·`tryIt` 규약은 [docs/views/apiDocs/data/README.md](../../docs/views/apiDocs/data/README.md).
- **대외용/개발자용 예제 분리**: 예제 노드에 `devOnly` 를 표기해 대외 문서에서 감춘다. 개발자용 모드는 `npm run dev_docs`(`--mode internal`) 또는 배포 환경에서 `?dev` 쿼리로 진입하며, 헤더·프라이머리 색상이 앰버로 구분된다.
