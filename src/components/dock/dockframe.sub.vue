  ...
  export default {
    ...
    activated() {
      ...
      if (this.addType === 'root') {
        ...
        this.addDataMapCrt();
        // id가 같으면 제일 첫 도킹 시도
      } else if (this.addId.id === this.insertId.id) {
        // 50% 사이즈를 줄인다.
    ...
    methods: {
      addInnerDataMapCrt() {
        ...
        const parentId = subDockFrameEl.getAttribute('id');
        const parentLayout = subDockFrameEl.className.match('hBox') ? 'hBox' : 'vBox';
        const subDock = {};

        subDock.pid = this.$el.parentElement.id;
        subDock.id = parentId;
      ...
      addInnerChildDataMap(subDockFrameEl, subDockFrameElLength, parentId, parentLayout) {
        for (let ix = 0; ix < subDockFrameElLength; ix++) {
          ...
          subDock.id = childDom.getAttribute('id');
          ...
          // subdock가 두개 생기는 시점부터 중복 된 노드 값은 추가하지 않는다.
            if (this.pos === 'right' || this.pos === 'bottom') {
              if (ix === 0) {
                this.nodeDataDelete(childDom.getAttribute('id'));
              }
            } else if (ix === 2) {
              this.nodeDataDelete(childDom.getAttribute('id'));
            }
        ...
        if (this.addPos !== '') {
          switch (this.addPos) {
            case 'left':
              ...
              if (targetEl.children[0].id.match('dockframe')) {
            ...
            if (!childDom.className.match('resizebar')) {
              ...
              if (childDom.childElementCount !== 0 && !childDom.className.match('DockContainer')) {
                if (childDom.className.match('vBox')) {
                  this.dockDomSize(childDom, 'vBox');
                } else if (childDom.className.match('hBox')) {
                  this.dockDomSize(childDom, 'hBox');
                }
              }
            }
          }
