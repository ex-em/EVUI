  ...
  const onCheck = (event, row) => {
    ...
    if (row[ROW_CHECK_INDEX]) {
      ...
      const isAllChecked = store
        ...
        .filter(rowData => !props.disabledRow.includes(rowData[ROW_DATA_INDEX]))
  ...
  const setStore = (rows, isMakeIndex = true) => {
    if (isMakeIndex) {
      ...
      rows.forEach((row, idx) => {
        ...
        const uncheckable = props.uncheckable.includes(row) || props.disabledRow.includes(row);
        let selected = false;
        if (selectInfo.useSelect) {
        ...
        const disabled = props.disabledRow.includes(row);
      ...
      checkInfo.isHeaderUncheckable = rows.every(row => props.uncheckable.includes(row)
        || props.disabledRow.includes(row));
