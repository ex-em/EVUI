    ...
    for (let x = 0; x < keys.length; x++) {
      ...
      for (let i = 0; i < storeLength; i++) {
        ...
        if (this.dataSet[key].fromTime <= xAxisTime) {
          let index = this.dataSet[key].endIndex - (this.dataSet[key].toTime - xAxisTime) / 1000;
          if (index < 0) {
  ...
  addSeriesStackDS(data, label, bsIds, sIdx = 0) {
    ...
    const getBaseDataPosition = (baseIndex, dataIndex, curr) => {
      ...
      const isPassingValue =
        !Util.isNullOrUndefined(baseSeries?.passingValue) && baseSeries?.passingValue === baseValue;
