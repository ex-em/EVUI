...
describe('model.store createRealTimeScatterDataSet 개별 series 만료 제거', () => {
  ...
  // 회귀: (b)를 `!datas[sId]?.length` 로 보면 축 패딩(y=null)만 보내는 소비자에서 만료가 영구히
  // 성립하지 않아 죽은 series 가 범례에 계속 남는다(lastTime 도 패딩 x 로 전진해 (a)까지 무력화).
  it('y=null 경계 패딩만 오는 series 는 활성으로 보지 않고 제거한다', () => {
    const store = buildStore({ range: 10 });
    const t0 = Math.floor(Date.now() / SECOND) * SECOND;
    ...
    for (let s = 1; s <= 10; s++) {
      const toTime = t0 + s * SECOND;
      store.createRealTimeScatterDataSet({
        a: [{ x: toTime, y: 1 }],
        b: [{ x: toTime, y: 2 }],
        c: [{ x: toTime, y: null }], // 축 우측단 유지용 패딩만
      });
    }
  ...
  // 전 대상 수신 중단 케이스: 값을 한 번도 받지 못한 series 는 축 패딩만으로 살아남지 않아야 한다.
  it('값을 한 번도 받지 못한 series 는 패딩만 와도 제거된다', () => {
    const store = buildStore({ range: 10, scatterIds: ['a', 'dead'] });
    const t0 = Math.floor(Date.now() / SECOND) * SECOND;
    ...
    store.createRealTimeScatterDataSet({
      a: [{ x: t0 + SECOND, y: 1 }],
      dead: [{ x: t0 + SECOND, y: null }],
    });

    expect(store.dataSet.dead).toBeUndefined();
    expect(store.dataSet.a).toBeDefined();
  });

  it('패딩과 값 점이 함께 오면 활성으로 보고 보존한다', () => {
    const store = buildStore({ range: 10 });
    const t0 = Math.floor(Date.now() / SECOND) * SECOND;
