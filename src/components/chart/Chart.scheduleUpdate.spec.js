 ...
 * (1) 인터랙션 직후 YIELD_AFTER_INTERACTION_MS(150) 양보와
 * (2) groupInteraction.deferUntil(deferPollingRedraw 가 설정) 까지의 양보를 max 로 합산한다.
...
describe('Chart.vue scheduleUpdate — deferUntil 폴링 양보', () => {
  it('deferUntil 미설정 + 인터랙션 과거: 데이터 변경이 즉시 update 를 발화한다(기존 0 delay)', async () => {
    const gi = { at: performance.now() - 1000, deferUntil: 0 };
  ...
  it('deferUntil 미설정 + 인터랙션 직후: 기존 150ms 양보가 보존된다', async () => {
    const gi = { at: performance.now(), deferUntil: 0 };
    const wrapper = mountChart({ groupInteraction: gi });
    await flushPromises();
    updateSpy.mockClear();
  ...
  it('deferUntil = now+1000: 1000ms 까지 미호출 후 정확히 1회 update', async () => {
    const gi = { at: performance.now() - 1000, deferUntil: performance.now() + 1000 };
    const wrapper = mountChart({ groupInteraction: gi });
    await flushPromises();
    updateSpy.mockClear();
  ...
  it('재무장: 타이머가 떠 있는 사이 deferUntil 을 연장하면 그만큼 더 지연된다', async () => {
    const gi = { at: performance.now() - 1000, deferUntil: performance.now() + 500 };
    const wrapper = mountChart({ groupInteraction: gi });
    await flushPromises();
    updateSpy.mockClear();
  ...
  it('deferUntil 이 과거면 즉시 update(기존 동일)', async () => {
    const gi = { at: performance.now() - 1000, deferUntil: performance.now() - 100 };
    const wrapper = mountChart({ groupInteraction: gi });
    await flushPromises();
    updateSpy.mockClear();
