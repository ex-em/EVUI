...
| 2026-07-31 | realTimeScatter 만료 판정의 좌단을 렌더 좌단(`fromTime + 1000`)에 맞추고, 만료·부활 판정 기준을 "값 있는 점(finite y)" 으로 통일 | 기존 `ds.toTime < windowStart` 는 렌더 좌단보다 1버킷 왼쪽이라, 점이 이미 안 보이는 틱에 제거되지 않고 한 틱 뒤에 제거됐다(외부 범례는 즉시 비워져 차트 범례와 한 틱 드리프트). 또 축 우측단 유지용 `y=null` 경계 패딩을 매 틱 보내는 소비자에서는 `lastTime` 이 계속 전진해 (a)·(b) 가 영구히 성립하지 않아 죽은 series 가 범례에 잔존했다 | 좌단만 보정(패딩 소비자의 영구 잔존은 그대로), 패딩 제거를 소비자에게 맡김(라이브러리가 방어 못 함), grace 틱 도입(지연이 더 커짐) |
