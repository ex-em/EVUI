 ...
 */
const createCore = ({ seriesList, listener, realTimeScatter = { use: true } }) => {
  const core = Object.create(EvChart.prototype);
  core.seriesList = seriesList;
  core.options = { realTimeScatter };
...
describe('EvChart.emitDataMaxChange', () => {
  ...
  it('realTimeScatter.use=false 면(그 외 차트 타입) emit 하지 않는다', () => {
    // 이벤트는 realTimeScatter 전용 — line/bar/pie 등은 게이트에서 일찍 빠져 집계 비용 0.
 ...
 * drawChart 가 실제로 그 메서드를 부르는 한 줄(chart.core.js:375)은 어떤 테스트로도 실행되지 않았다.
...
describe('EvChart.drawChart → emitDataMaxChange 호출부 배선', () => {
  const createDrawCore = ({ seriesList, listener }) => {
    const core = Object.create(EvChart.prototype);
    core.options = { realTimeScatter: { use: true } };
    core.seriesList = seriesList;
