...
- **커스텀 HTML 툴팁**: `tooltip.formatter.html(seriesList)`이 반환한 HTML을 파싱해 tooltip DOM에 부착(`drawCustomTooltip`). 루트 노드는 항상 `tooltipDOM.firstElementChild`로 취급하며 위치 계산(`setCustomTooltipLayoutPosition`)은 그 크기 기준. 드래그 중(`dragSelection.use`)에는 2번째 인자로 `{ dragRange: { from, to } }`를 함께 넘긴다 — mouseup 때 `drag-select` 가 내보낼 range 를 **같은 함수(`getSelectionRange`/heatMap 은 `getSelectionRangeForHeatMap`)로 구해** x 성분만 쓰므로 라이브 헤더와 최종 페이로드가 같은 값이다. 드래그 중이 아니면 인자를 넘기지 않아 기존 formatter 의 arity 가 유지된다. 드래그 중 hit 이 0개인 프레임(막대 사이 간격·all-null 라벨)은 `dragSelection.showTooltipOnEmpty` 가 켜졌을 때만 빈 seriesList 로 호출해 구간 헤더만 남긴다 — 기본값은 기존대로 감춘다. `tooltip.returnValue` 는 소비처가 직접 렌더하는 경로라 제외한다. 이렇게 띄운 툴팁은 `dragEnd` 가 즉시 감춘다 — `keepDisplay: false` 면 밴드까지 지워져 근거 없는 헤더만 남는다. `formatter.html` 이 던지는 예외는 호출 지점에서 가두고 툴팁만 감춘다(콘솔 경고 인스턴스당 1회) — 그대로 올리면 호출부가 overlay 를 비운 뒤라 하이라이트·드래그 밴드까지 사라지고, `restoreDragArtifacts` 경로에서는 재렌더 예약 플래그가 정리되지 않는다.
- **커스텀 툴팁 가상 스크롤**: `tooltip.virtualScroll` 옵션(use: true/'auto'+threshold 기본 50)에 따라 formatter.html 결과에서 row 컨테이너를 탐지(① `[data-evui-tooltip-row]` 속성 행 개수=시리즈 수+동일 부모, ② BFS로 자식 수=시리즈 수인 가장 얕은 요소)하여 상/하 spacer + viewport 구조로 재구성. prefix sum 기반 가시 범위 계산, 실측 높이 학습(행 아이템만), 앵커 보정으로 스크롤 점프 방지, ResizeObserver로 폭 변화 시 측정 무효화. 탐지 실패 시 전체 부착 경로로 fallback(콘솔 경고 1회, `_vsDetectFailed`로 같은 마크업 동안 재시도 차단 — `render()`에서 리셋). formatter 예외도 같은 fallback 이지만 경고 1회만 묶고 재시도는 차단하지 않는다 — 일시적으로 던지는 formatter 까지 전체 부착 경로에 고정하는 비용이 더 크다.
...
- **indicator 히트 영역**(`getIndicatorHitBounds`): 도메인 축(일반 차트 X / horizontal 차트 Y)은 `axesSteps.graphMin~graphMax` 대비 `axesRange.min~max` 비율로 환산한 **데이터 존재 픽셀 구간**까지만 hit으로 인정해 빈 구간에서 indicator가 뜨지 않게 한다(다축이면 합집합). 문자열 라벨 축처럼 비율 계산이 불가능한 축이 **하나라도 있으면**(`calcDomainBounds` → null) 그 축의 점유 구간을 알 수 없으므로 좁히지 않고 차트 경계로 fallback. 교차 축만 기존 ±15px tolerance 유지. 선 길이는 항상 차트 경계(`x1~x2`/`y1~y2`).
...
- **hover fast path**: hitId+시리즈/dataIndex 시그니처(`hoverSig`)가 직전과 같고 툴팁이 표시 중이면 `drawCustomTooltip`(formatter.html 경로의 최고 비용)만 스킵. overlay/indicator/listener 흐름은 유지. mouseleave·데이터 갱신 시 시그니처 무효화. 드래그 중 프레임은 fast path 를 쓰지 않는다 — 헤더의 구간 값이 커서 픽셀에 연속으로 의존해서, 같은 데이터 포인트 위에서 스킵하면 헤더가 라벨 간격만큼 뒤처진 채 얼어붙는다. 시그니처에는 `d|` 접두를 붙여 드래그 종료 직후 첫 hover 도 다시 그리게 한다.
...
- **드래그 선택**: `dragSelection.use` + scatter/line/heatMap/수직 bar/콤보(`horizontal: false`)에서 드래그로 영역 선택. 콤보는 `options.type`이 없고 시리즈가 각자 `type`을 선언하는 구성이라 게이트가 falsy type으로 판정한다 — `options.type: 'pie'`인 pie는 걸리지 않지만, `options.type` 없이 시리즈 레벨 `type`으로만 pie·heatMap을 선언한 구성은 통과한다. 둘 다 온전한 구성이 못 된다 — heatMap은 `data.labels`가 `{ x, y }` 형태여야 한다 — 콤보의 평면 `labels`가 그대로 시리즈에 실려(`model.store.js:45`) `computeGeometry`의 `labels.x.length`에서 터지고, 축 라벨도 `options.type === 'heatMap'`일 때만 `data.labels[dir]`로 읽힌다. pie는 축을 선언하지 않아 `getSelectionRange`가 `null`을 돌려주고, `dragEnd`의 zoom 위임 분기는 그 `null`을 만나면 위임을 건너뛴다. 시리즈 레벨 heatMap은 `dragEnd`의 heatMap 분기도 비껴가 블록 스냅 대신 line식 `getSelectionRange`를 받는다. `options.type`을 두고 일부 시리즈만 `type`으로 덮은 구성은 그 타입(line·scatter·bar)으로 이미 통과한다. 누적 막대는 차트 타입이 아니라 `data.groups`로 표현되고 게이트는 `options`만 읽으므로 일반 막대와 동일하게 동작한다. 차트 영역으로 clamp된 rect를 계산해 `drag-select` 리스너에 `{ data: findSelectedItems, range: getSelectionRange }` 전달(zoom.use 시엔 `zoom.getRangeInfo`로 위임). line·bar는 y축 전체 높이 고정, heatMap은 `findBlockRange`로 블록 단위 스냅. `findSelectedItems`는 `options.type`을 읽지 않고 `seriesList` 전부에 `findItems`를 호출하므로, 시리즈 `type` 오버라이드로 bar를 섞은 line·scatter 차트의 `data`에도 그 막대가 실린다(아이템이 없는 시리즈는 제외). `Bar.findItems`는 막대의 x 구간이 드래그 구간에 **걸치기만 해도** 담는다(막대는 폭을 가지므로 완전 포함을 요구하지 않는다). 값이 `null`이거나 축 범위를 넘은 항목도 담긴다 — 수직 막대의 `xp`는 라벨 기준이라 값과 무관하게 채워지고 그 신호는 `h`에만 실린다(line과 동일 계약). 대상은 가시 윈도우(`visibleStartIndex` ~ `filteredCount`)로 좁힌다 — 스크롤바 이동은 `lightUpdate`라 윈도우 밖 항목의 `xp`가 직전 렌더값으로 남아, 좌표 존재 여부만으로는 걸러지지 않는다. 숨긴 시리즈·가로 막대는 제외한다 — 이 제외 규칙은 `Bar` 한정이고 `Line.findItems`에는 `show` 가드가 없어 범례로 끈 line 시리즈의 이전 데이터가 `data`에 실릴 수 있다. 범주형(categoryMode) bar의 range는 인덱스 슬롯 배치가 아닌 `graphMin..graphMax` 선형 보간이라 최대 약 1 막대 폭 오차를 갖는다. 누적 bar는 시리즈 `.y`가 누적 합이므로(`addSeriesStackDS` → `getSeriesMinMax`) y축 range도 스택 총합 공간이다 — 개별 시리즈 값으로 읽으면 어긋난다(x는 누적과 무관). zoom 위임 시에도 같은 부분 겹침 규칙이 쓰여, 줌 창이 시각적 밴드보다 양 끝 최대 1 인덱스 넓어질 수 있다. `keepDisplay` 시 선택 영역 잔존(`dragInfoBackup`) + resize 시 range 비율로 재스케일. 드래그 중에도 커서 위치의 툴팁·인디케이터·하이라이트가 갱신된다 — `dragMove` 가 `drawHoverArtifacts` → `drawSelectionArea` 순으로 그려 밴드가 hover 위에 남는다(커서가 캔버스 밖이면 툴팁을 감추고 hover 없이 clear + 밴드만 — 밴드는 clamp 좌표로 계속 갱신되므로 남겨둔 툴팁은 밴드와 다른 구간을 가리킨다). 드래그 경로의 hide 는 `debouncedHide` 를 우회한 즉시 hide(`hideTooltip`)를 쓴다 — trailing 디바운스는 프레임마다 타이머가 리셋돼 끝까지 감춰지지 않는다. 모바일 scatter는 click 시 `size`(기본 50px) 터치 박스 선택.
...
2. **툴팁 DOM 위치**: tooltip DOM(`.ev-chart-tooltip`)은 `document.body` 직속이고(`createTooltipDOM`), 배치는 `transform: translate3d`로만 이동한다(`top/left`는 0 고정 — 레이아웃/리페인트 회피). 좌표는 커서의 문서 좌표(`e.pageX/pageY`)이며, 반전 판정의 기준은 **문서 좌표로 환산한 가시 영역**(`getVisibleDocumentRect` = `scrollX/Y + documentElement.clientWidth/Height`)이다 — `document.body.clientWidth/Height`가 아니다(body가 뷰포트보다 넓으면 한계가 화면 밖이라 반전이 통째로 건너뛰어진다). 캔버스 툴팁과 커스텀 HTML 툴팁은 `placeTooltipDOM` 한 곳을 공유한다. **가로 반전 방향은 직전 방향을 유지한다**(히스테리시스) — 매 호출의 폭으로 다시 판정하면, hover 지점마다 폭이 달라지는 차트에서 커서가 한 방향으로 움직이는 동안에도 좌우가 번갈아 뒤집힌다. 지금 방향으로 두면 가시 영역을 벗어나는 경우에만 전환한다. **양쪽 모두 안 들어가면(툴팁이 가시 영역보다 넓음) 히스테리시스 대신 남은 폭이 넓은 쪽을 택한다** — 커서 위치만 보는 판정이라 폭 변동에 흔들리지 않고, 두 여유의 합이 항상 `가시 폭 - 40` 이라 상한이 0 이하가 되지 않는다. 폭만 보고 한쪽을 고정하면 커서가 그 반대편 20px 안에 있을 때 상한이 0 이 돼 내용이 통째로 사라진다. 판정에 쓰는 폭은 콘텐츠가 아니라 **tooltipDOM 실측 폭**이다 — 상한이 tooltipDOM 에 걸리고, 인라인 `border: 1px`·header padding 때문에 콘텐츠 폭보다 넓다. 숨김 경로(`hideTooltipDOM`·`tooltipClear`)는 `resetTooltipPlacement` 로 방향 기억을 버려, 다시 표시될 때 기본 방향(커서 오른쪽)에서 시작한다 — mouse-leave 의 주 경로는 `tooltipClear` 라 둘 다 필요하다. 반대로 `placeTooltipDOM` 은 예약된 `debouncedHide` 를 취소한다 — 트레일링 콜백이 살아있으면 보이는 툴팁을 숨기고 방향까지 지운다. **세로는 방향 기억 없이 매번 판정하고, 폭과 달리 높이 상한을 걸지 않는다** — 대신 배치 좌표에 가시 영역 상단을 하한으로 건다(`Math.max(view.top, …)`). 상한이 없으므로 위 공간보다 높은 툴팁은 반전 좌표가 음수가 돼 상단이 잘리고, 그 손실은 스크롤로 되돌릴 수 없다 — 아래 방향은 문서 좌표라 넘쳐도 스크롤로 닿으므로 하한만 둔다. **위·아래 모두 안 들어가면 가로와 같이 남은 공간이 넓은 쪽을 택한다** — 아래가 넓은데도 위로 반전하면 툴팁이 커서와 차트를 덮는다. **반전 배치는 좌표가 아니라 `translateX(-100%)` 로 우단을 앵커(`커서 - 20px`)에 고정한다** — 좌측을 `커서 - 폭 - 20` 으로 잡으면 배치 이후 폭이 커지는 경로에서 툴팁이 커서 쪽으로 자라 커서를 덮는다. 우단을 고정하면 폭 증가가 왼쪽으로만 향한다. 폭 상한도 앵커 기준이다 — 반전이면 앵커에서 가시 영역 좌단까지, 아니면 앵커에서 우단까지.
3. **툴팁은 문서 가로 스크롤을 늘리지 않는다**: body 직속 `absolute`라 tooltipDOM이 가시 영역 우단을 넘은 폭이 그대로 문서 스크롤 폭이 된다. 배치 시 **그 지점부터 우단까지 남은 폭**을 `max-width`로 걸어, 배치 이후 내용이 넓어지는 경로(가상 스크롤 rAF 렌더, 내부 스크롤바 출현, 웹폰트 적용, 라이브 갱신)에서도 레이아웃 단계에서 우단을 넘지 못하게 한다 — 사후 교정(ResizeObserver 등)은 한 프레임 늦어 넘친 프레임이 그대로 그려진다. 배치 방향은 `fits*` 판정을 통과한 쪽이므로 상한은 항상 실측 폭 이상이라 내용을 좁히지 않는다(= `overflow-x: hidden` 클리핑을 유발하지 않는다). 단, 양쪽 모두 안 들어가는 경우는 예외다 — 넓은 쪽 여유로 잠기므로 내용이 잘린다(툴팁이 화면보다 넓으면 잘리지 않을 방법이 없다). **상한이 실제로 구속되는 이 구간에서만 16px 단위로 내림한다**(`quantizeTooltipMaxWidth`) — 상한은 커서 좌표의 함수라 1px 이동마다 값이 달라지는데, 구속되는 구간에서는 그 값이 곧 used width 라 가상 스크롤의 폭 감시 ResizeObserver 가 mousemove 마다 행 측정을 전량 무효화한다. 내림이라 남은 폭을 넘지 않아 넘침 차단은 그대로다. 구속되지 않는 구간에 적용하면 상한이 실측 폭 아래로 내려가 위 계약이 깨지고, 없던 RO 발화가 새로 생긴다. `.ev-chart-tooltip`의 `box-sizing: border-box`는 인라인으로 붙는 `border: 1px`가 상한 안에 들어오게 한다(호스트 앱 전역 리셋에 의존하지 않기 위함). **세로에는 상한을 걸지 않는다** — 반전만 하며, 배치 이후 높이가 커지는 경우는 막지 않는다. 세로 반전 판정에 쓰는 높이도 **상한 적용 전** 실측값이다 — 상한이 구속돼 줄바꿈이 생기면 실제 높이는 그보다 커질 수 있으나, 재측정은 강제 레이아웃을 한 번 더 만들고 세로 넘침은 문서 좌표라 스크롤로 닿으므로 받아들인다. 상한이 다음 배치의 자연 크기 측정을 누르지 않도록, 각 진입점(`setTooltipLayoutPosition` / `setCustomTooltipLayoutPosition`)은 측정 직전에 이를 해제한다. 커스텀 경로는 `drawCustomTooltip`(가상 스크롤 행 높이 실측 포함)이 배치보다 먼저 실행되므로 해제 지점을 `drawCustomTooltip` 진입부에도 둔다 — 그렇지 않으면 행 높이가 직전 배치의 상한 아래(줄바꿈된 높이)에서 굳는다.
4. **범례 최소 활성 보장**: `clickMode !== 'active'`에서 활성 범례가 1개면 클릭해도 비활성화되지 않는다(일반 범례는 `seriesInfo.count === 1`, heatMap 색상 범례는 `activeCount === 1` 기준).
5. **범례 테이블 제외 타입**: legend table은 heatMap/scatter 타입에서 사용하지 않는다. 단, 판정 문자열이 chart.core.js(update)는 `'heatMap'`, plugins.legend.js(initLegend)는 `'heatmap'`(소문자)로 서로 다르다. [NEEDS CLARIFICATION: 차트 타입 표기는 `'heatMap'`인데 initLegend의 useTable 판정만 `'heatmap'` 비교라 heatMap+legend.table.use 조합에서 init과 update의 useTable 판정이 갈린다 — 의도인가 오타인가?]
6. **initEvent 1회 원칙**: 범례 이벤트 리스너(click/mouseover/mouseleave)는 `isInitLegend` 가드로 최초 1회만 등록된다. `forceUpdateLegend`는 destroy 후 재-init하므로 이때만 재등록된다.
7. **스크롤바 wheel 등록 주체 이원화**: 스크롤바 wheel(`onScrollbarWheel`)은 overlayCanvas에 스크롤바 존재 시 등록되고, 툴팁 내부 스크롤 wheel(`onWheel`, interaction)은 `tooltip.useScrollbar` 또는 가상 스크롤 가능 조건에서 등록된다. 둘 다 툴팁 열림+스크롤 여지 시 툴팁 스크롤을 우선한다.
8. **plot 라벨 우선**: 커서가 plot 라벨 박스 안이면 그 프레임의 series hit을 비워 series 툴팁/하이라이트를 끄고 라벨 툴팁만 표시한다(데스크탑 전용 — `isMobile`이면 onMouseMove 조기 반환).
9. **가상 스크롤 행 분류 기준**: 행/비-row 분류는 오직 `data-evui-tooltip-row` 속성 유무이며 클래스명은 보지 않는다. 마크된 행이 하나도 없으면(BFS 경로) 전부 행으로 간주한다. 높이 학습 평균은 행 아이템만 대상으로 계산한다.
10. **드래그 선택 활성화**: 캔버스 밖(startArea)에서 시작한 드래그는 포인터가 캔버스에 진입한 순간 활성화되며 진입 전에는 preventDefault를 호출하지 않는다(단, 전용 표시 캔버스가 있으면 시작 즉시 활성화). 드래그 종료는 window mouseup에서 처리하고 `width > 1 && height > 1`일 때만 drag-select를 발화한다.
11. **선택 상태와 리스너**: click 리스너는 `dragInfoBackup`(드래그 선택 잔존)이 있으면 발화하지 않는다. `selectSeriesByData`는 마지막 render된 선택(`_renderedSelectSeriesIds`)과 동일하면 render를 skip한다(차트 그룹 deep watch 스팸 차단).
...
15. **드래그 중 overlay 소유권**: 드래그가 시작되면 overlayCanvas 의 clear→draw 는 window mousemove(`dragMove`) 한 곳이 소유한다. 드래그 중 hover 갱신 자체는 `dragSelection.updateHoverOnDrag`(기본 false) 옵션이고, 끄면 `dragMove` 는 overlay 를 비우고 밴드만 그린다 — 하이라이트·인디케이터는 남지 않고(overlay 는 `onMouseDown` 의 `removeSelectionArea` 에서 이미 비워진다), tooltipDOM 은 별개라 라이브러리가 건드리지 않는다 — 이하 규약은 옵션이 켜진 경우다. canvas mousemove(`onMouseMove`)는 기존대로 조기 반환하고, `dragMove` 가 `drawHoverArtifacts` → `drawSelectionArea` 를 한 프레임 안에서 호출한다. 이 경로에는 `tooltip.throttledMove` 를 적용하지 않는다 — clear 와 draw 가 프레임을 넘나들면 hover 또는 밴드가 깜빡인다. 드래그 중에는 `mouse-move` 를 발화하지 않으므로(기존 동작 유지) 차트 그룹 indicator 동기화는 드래그 동안 멈춘다. 커스텀 툴팁 redraw 스킵(hoverSig fast path)도 드래그 중에는 꺼지므로 프레임당 formatter 1회 비용이 든다 — 헤더가 커서에 연속으로 의존하는 대가다. 그 1회를 지키려고 `setCustomTooltipLayoutPosition` 의 빈-루트 복구 재draw 는 드래그 중에 건너뛴다 — 복구는 redraw 스킵이 켜진 프레임을 위한 것이라, 스킵이 꺼진 드래그 프레임에서는 같은 결과를 다시 만들 뿐이다. 예외를 던지는 formatter 만 가상 경로가 비가상 경로로 fallback 해 프레임당 2회가 되고, 경고는 1회로 묶는다. 드래그 도중 재렌더가 일어나면 `clear()` 가 overlay 를 항상 비우고 `updateTooltip` 이면 tooltipDOM 까지 비우므로 `restoreDragArtifacts`(chart.core)가 `dragMove` 가 남긴 `lastDragHoverEvent` 로 같은 순서를 한 번 더 재생한다 — 그 이벤트는 커서가 캔버스 안이었던 프레임에만 남고 `dragEnd` 가 지운다.
...
- 콤보(`options.type` undefined)는 scatter·heatMap 분기를 비껴가 line·bar와 같은 y 밴드·range 경로를 타고, bar·line 시리즈를 한 페이로드에 담는다. (plugins.interaction.spec.js)
...
- 축을 선언하지 않아 `axesSteps`가 빈 구성은 `range`가 `null`이고, 리스너 미바인딩·zoom 위임 두 경로 모두 mouseup이 터지지 않는다(zoom 위임은 건너뛴다). (plugins.interaction.spec.js)
...
- `dragSelection.use` + scatter/line/heatMap/수직 bar/콤보(`options.type` 미지정)에서만 mousedown이 드래그를 시작한다. `horizontal: true` bar·콤보와 `options.type: 'pie'`인 pie는 시작하지 않는다. `data.groups`(누적)는 판정에 관여하지 않아 누적 수직 bar도 시작하고, 누적 + `horizontal: true`는 시작하지 않는다. (plugins.interaction.spec.js)
...
- `dragSelection.use` + `updateHoverOnDrag` line·수직 bar·콤보 차트에서 드래그하는 동안 커서 위치의 툴팁/하이라이트가 갱신되고 선택 밴드가 그 위에 유지된다. 커서가 캔버스 밖이거나 모바일이면 툴팁을 감추고 밴드만 그린다. 커스텀 툴팁 formatter 는 드래그 중에만 2번째 인자로 `{ dragRange }` 를 받는다. (plugins.dragSelection.hover.spec.js)
- `updateHoverOnDrag` 가 꺼져 있으면(기본값) 드래그 프레임은 overlay 를 비우고 밴드만 그린다 — hover 도 툴팁 hide 도 없고 `lastDragHoverEvent` 도 남지 않는다. (plugins.dragSelection.hover.spec.js)
...
- 드래그 프레임에서 커스텀 툴팁 루트가 비어도 `setCustomTooltipLayoutPosition` 이 복구 재draw 를 하지 않는다 — 그 프레임의 `formatter.html` 호출 수를 늘리지 않는다. 비드래그 hover 는 그대로 복구한다. (plugins.dragSelection.hover.spec.js)
- 가상 스크롤 경로에서 `formatter.html` 이 던지면 매번 비가상 경로로 fallback 하되 콘솔 경고는 인스턴스당 1회다. (plugins.tooltip.virtualScroll.spec.js)
...
| 파일 | 역할 |
|------|------|
| plugins.interaction.js | 마우스/터치 이벤트 파이프라인(onMouseMove/Leave/Click/DblClick/MouseDown/Wheel, hover 렌더 본체 drawHoverArtifacts), hit-test(findHitItem, findClosestDataIndex, buildLabelValidMask), 드래그 선택(dragStart, drawSelectionArea, 전용 표시 캔버스), 아이템/라벨/시리즈 선택 상태 관리 및 외부 선택 API, 툴팁 label/value 포맷(+WeakMap 캐시) |
