...
- **드래그 선택**: `dragSelection.use` + scatter/line/heatMap/수직 bar(`horizontal: false`)에서 드래그로 영역 선택. 차트 영역으로 clamp된 rect를 계산해 `drag-select` 리스너에 `{ data: findSelectedItems, range: getSelectionRange }` 전달(zoom.use 시엔 `zoom.getRangeInfo`로 위임). line·bar는 y축 전체 높이 고정, heatMap은 `findBlockRange`로 블록 단위 스냅. `Bar`에 `findItems`가 없어 bar 시리즈는 `data`에 담기지 않는다(같은 차트에 섞인 line 등 `findItems` 구현 시리즈만 수집). 범주형(categoryMode) bar의 range는 인덱스 슬롯 배치가 아닌 `graphMin..graphMax` 선형 보간이라 최대 약 1 막대 폭 오차를 갖는다. `keepDisplay` 시 선택 영역 잔존(`dragInfoBackup`) + resize 시 range 비율로 재스케일. 모바일 scatter는 click 시 `size`(기본 50px) 터치 박스 선택.
...
- `dragSelection.use` + scatter/line/heatMap/수직 bar에서만 mousedown이 드래그를 시작한다. `horizontal: true` bar는 시작하지 않는다. (plugins.interaction.spec.js)
