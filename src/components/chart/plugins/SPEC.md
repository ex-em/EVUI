...
- **hover fast path**: hitId+시리즈/dataIndex 시그니처(`hoverSig`)가 직전과 같고 툴팁이 표시 중이면 `drawCustomTooltip`(formatter.html 경로의 최고 비용)만 스킵. overlay/indicator/listener 흐름은 유지. mouseleave·데이터 갱신 시 시그니처 무효화. 드래그 중 프레임은 시그니처에 `d|` 접두를 붙여 드래그 진입/종료 프레임이 같은 데이터 포인트에서도 매치되지 않게 한다(구간 정보가 formatter 인자로 함께 들어가므로).
