...
- **툴팁 값 포맷 캐시**: `getFormattedTooltipValue`는 itemData 객체를 키로 한 WeakMap에 seriesId별 포맷 결과를 캐시. point 객체는 풀링되어 데이터 갱신 시 in-place 로 덮어써지므로(model.store `addData` target 재사용) 자동 GC 무효화가 성립하지 않는다 — chart.core `update()`가 updateData/updateSeries 시 캐시를 명시적으로 비운다(`_lastHoverSig` 리셋과 동일 지점). pie/heatMap/기본 타입별 formatter 인자 형태 상이.
- **드래그 선택**: `dragSelection.use` + scatter/line/heatMap에서 드래그로 영역 선택. 차트 영역으로 clamp된 rect를 계산해 `drag-select` 리스너에 `{ data: findSelectedItems, range: getSelectionRange }` 전달(zoom.use 시엔 `zoom.getRangeInfo`로 위임). line은 y축 전체 높이 고정, heatMap은 `findBlockRange`로 블록 단위 스냅. `keepDisplay` 시 선택 영역 잔존(`dragInfoBackup`) + resize 시 range 비율로 재스케일. 모바일 scatter는 click 시 `size`(기본 50px) 터치 박스 선택.
