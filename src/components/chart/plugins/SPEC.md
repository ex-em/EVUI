...
- `dragSelection.use` + scatter/line/heatMap/수직 bar/콤보(`options.type` 미지정)에서만 mousedown이 드래그를 시작한다. `horizontal: true` bar·콤보와 pie는 시작하지 않는다. `data.groups`(누적)는 판정에 관여하지 않아 누적 수직 bar도 시작하고, 누적 + `horizontal: true`는 시작하지 않는다. (plugins.interaction.spec.js)
