...
const modules = {
  ...
  getFormattedTooltipValue({ dataId, seriesId, seriesName, value, itemData }) {
    ...
    // _dataEpoch 를 받지 못하는 경로에선 캐시하지 않는다(computeGeometry 의 canMemo 가드와
    // 동일 취지) — realTimeScatter 는 createRealTimeScatterDataSet 별도 경로라 epoch 가
    // undefined 로 남는데, 점객체 값은 불변이지만 formatter 런타임 교체를 무효화할 방법이
    // 없으므로 캐시를 끈다(scatter hover hit 은 mousemove 당 점 1개라 비용 미미).
    const useCache =
      itemData !== null &&
      typeof itemData === 'object' &&
      tooltipValueFormatter &&
      this._dataEpoch != null;
    if (useCache) {
