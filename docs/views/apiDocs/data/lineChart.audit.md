...
우선순위: **코드 > md > JSON**. md와 코드가 다르면 코드가 맞다고 가정한다.

**문서화 원칙: 코드에 존재하는 공개 옵션·이벤트·슬롯은 예외 없이 문서(md·JSON 모두)에 표시되어야 한다.**
"의도적 생략"은 인정하지 않는다. 유일한 예외는 별도 문서로 분리 예정인 영역(zoom/brush → EvChart(Zoom)/EvChart(Brush))과
다른 차트 타입 전용 옵션(scatter/heatMap 전용 등)뿐이다. 코드에 있는데 문서에 없으면 무조건 '누락(mid 이상)'으로 보고한다.
