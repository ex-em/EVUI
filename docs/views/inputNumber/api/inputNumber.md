...
| clamp-on-step | Boolean | false | step으로 인해 `min`/`max`를 초과할 경우 값 변경을 막는 대신 `min` 또는 `max` 값으로 변경됨. `step-strictly`와 함께 사용하면 경계값(`min`/`max`)이 step 그리드로 다시 스냅되므로, `min`/`max`가 step 배수가 아니면 경계에 정확히 도달하지 않고 범위 내 가장 가까운 step 값까지만 변경됨(예: `min=0, max=10, step=3` → 도달 가능한 최댓값은 `9`) | true, false |
