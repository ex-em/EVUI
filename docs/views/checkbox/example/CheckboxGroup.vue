<template>
  <div class="case">
    ...
    <ev-checkbox-group v-model="checkboxGroup">
      <ev-checkbox label="Option A" tooltip-title="1번 옵션"> A </ev-checkbox>
      <ev-checkbox label="Option B" tooltip-title="2번 옵션" />
      <div>
        <ev-checkbox label="Option C" tooltip-title="3번 옵션" />
      </div>
      <div>
        <ev-checkbox label="Option D" tooltip-title="4번 옵션" />
      </div>
    </ev-checkbox-group>
    <div class="description">
      <span class="badge"> Checkbox Group Values </span>
      {{ checkboxGroup }}
    </div>
  </div>
  <div class="case">
    <p class="case-title">Event</p>
    <ev-checkbox-group v-model="checkboxGroup2" @change="changeGroup2">
    ...
    <div class="description">
      <span class="badge"> Checkbox Group Values </span>
      <button class="btn" @click="clickButton">Add 'Option A'</button>
      &nbsp; {{ checkboxGroup2 }}
    </div>
    <div class="description">
      <span class="badge"> Checkbox Group Change Event </span>
      &nbsp; {{ changeEventVal }}
    </div>
  </div>
  <div class="case">
    <p class="case-title">All Check</p>
    <ev-checkbox-group v-model="checkboxGroup3" @change="changeGroupValues">
      <ev-checkbox
  ...
  <div class="case">
    ...
    <ev-checkbox-group v-model="buttonGroup" type="button">
    ...
    <br />
    <ev-checkbox-group v-model="buttonGroup" type="button">
      <ev-checkbox label="Option A" disabled />
      <ev-checkbox label="Option B" disabled />
      <ev-checkbox label="Option C" />
    </ev-checkbox-group>
  </div>
</template>
...
export default {
  setup() {
    ...
    const changeGroup2 = (val) => {
      changeEventVal.value = val;
    };
