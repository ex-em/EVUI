...
| showTooltipOnEmpty | Boolean | false | 드래그 중 커서 위치에 데이터가 없어 hit이 0개인 프레임에서도 `tooltip.formatter.html`을 빈 seriesList와 `{ dragRange }`로 호출해 구간 헤더만 남깁니다. `formatter.html` 경로 전용이며 `tooltip.returnValue`를 쓰면 동작하지 않습니다. | true / false |
