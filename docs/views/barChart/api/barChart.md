...
| drag-select | data, range | 드래그로 영역을 선택했을 때 발생. `range`는 선택 영역의 `{ xMin, xMax, yMin, yMax }`다. `data`에는 드래그 구간에 걸친 막대가 담긴다 — 막대는 폭이 있으므로 구간에 완전히 들어가지 않고 일부만 걸쳐도 포함된다. 누적 막대는 각 포인트가 `y`(누적 합)와 `o`(자기 값)를 함께 갖는다. 범주형 x축에서 `range`는 막대의 슬롯 위치가 아니라 축 범위 위 선형 보간으로 계산되므로 최대 약 1 막대 폭의 오차가 있을 수 있다. `dragSelection.use`가 true이고 `horizontal`이 false일 때만 발생한다. 누적 막대에서도 동일하게 발생하며, `range`의 y 값은 스택 총합 기준이다. |
