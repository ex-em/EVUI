...
##### Bar 차트의 range 동작 (카테고리 축)

Bar 차트는 데이터 배열의 **인덱스 순서로 막대를 균등 분포**합니다.<br>
값 축(수직 막대 기준 y축, 수평 막대 기준 x축)의 `range`는 모든 scale에서 정상 동작합니다.<br>
카테고리 축(수직 막대 기준 x축, 수평 막대 기준 y축)의 `range`는 scale 유형에 따라 동작이 다릅니다.

- `step`, `time` + `categoryMode: true`<br>
  range로 지정한 인덱스/시간 구간 안의 데이터만 그려집니다.
- `time` (`categoryMode: false`), `linear`<br>
  range로 지정한 시간/값 영역 안의 데이터만 그려집니다.<br>
  단, **막대 위치는 남은 데이터의 인덱스 균등 분포**입니다. 이로 인해:
  - range 내 데이터가 **균일하게 샘플링**되어 있을 때 시간/값 라벨과 막대 위치가 자연스럽게 정렬됩니다.
  - 불균일 샘플링(예: 일부 시간대 데이터 누락)이면 라벨과 막대 위치가 일치하지 않을 수 있습니다.
  - `range`가 `labels` 범위보다 **넓은 경우** 라벨은 range 전체 영역에 표시되지만 막대는 가시 데이터에만 균등 분포되어 어긋날 수 있습니다. 자연스러운 정렬을 위해 `range`는 `labels` 범위 안에서 지정하는 것을 권장합니다.
