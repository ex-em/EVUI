...
const useModel = (props: Props, emit: Emit) => {
  ...
  const numberToUnit = (input?: string | number) => {
    let output;
    let result;

    if (typeof input === 'string' || typeof input === 'number') {
      const match = /^(normal|(-*\d+(?:\.\d+)?)(px|%|vw|vh)?)$/.exec(
        input.toString()
      );
      output = match
  ...
  const removeUnit = (input?: number | string, direction = 'horizontal') => {
  ...
  // close window
  const closeWin = (from?: 'layer' | MouseEvent) => {
  ...
  const changeBodyCls = (isVisible?: boolean) => {
...
const useMouseEvent = (
  ...
  const dragStyle = reactive<DragStyleInfo>({
    top: 0,
    left: 0,
    width: 0,
    height: 0,
  ...
  const clickedInfo = reactive<ClickedInfo>({
    state: '',
    pressedSpot: '',
    top: 0,
    left: 0,
  ...
  const beforeExpandPosInfo = reactive<ExpandPosInfo>({});

  const isInHeader = (x: number, y: number) => {
    if (x == null || y == null) {
      return false;
    }

    const rect = windowRef.value?.getBoundingClientRect();
    const posX = +x - (rect?.left ?? 0);
    const posY = +y - (rect?.top ?? 0);
    const headerAreaStyleInfo = headerRef.value?.style;
    const headerPaddingInfo = {
      top: removeUnit(headerAreaStyleInfo?.paddingTop ?? 0, 'vertical'),
      left: removeUnit(headerAreaStyleInfo?.paddingLeft ?? 0, 'horizontal'),
      right: removeUnit(headerAreaStyleInfo?.paddingRight ?? 0, 'horizontal'),
    };
    const startPosX = headerPaddingInfo.left;
    const endPosX = rect?.width ?? 0 - headerPaddingInfo.right;
    const startPosY = headerPaddingInfo.top;
    const endPosY = startPosY + (headerRef.value?.offsetHeight ?? 0);
  ...
  const setDragStyle = (paramObj: Partial<DragStyleInfo>) => {
    ...
    dragStyle.top = numberToUnit(top) ?? 0;
    dragStyle.left = numberToUnit(left) ?? 0;
    dragStyle.width = numberToUnit(width) ?? 0;
    dragStyle.height = numberToUnit(height) ?? 0;
    dragStyle.minWidth = numberToUnit(tMinWidth) ?? 0;
    dragStyle.minHeight = numberToUnit(tMinHeight) ?? 0;
  };

  const changeMouseCursor = (e: MouseEvent) => {
  ...
  // 브라우저 상하 위치 제약
  const getValidTop = (windowHeight: number, top: number) => {
    let tempTop = top;
  ...
  // 브라우저 좌우 위치 제약
  const getValidLeft = (windowWidth: number, left: number) => {
    let tempLeft = left;
  ...
  const startDrag = (e: MouseEvent) => {
    ...
    let pressedSpot: ClickedInfo['pressedSpot'] = '';
    if (props.resizable) {
  ...
  const initWindowInfo = () => {
    ...
    beforeExpandPosInfo.top = undefined;
    beforeExpandPosInfo.left = undefined;
    beforeExpandPosInfo.width = undefined;
    beforeExpandPosInfo.height = undefined;

    Object.keys(dragStyle).forEach((key) => {
      delete dragStyle[key as keyof DragStyleInfo];
    });
  };
...
const activeWindows = (() => {
  ...
  return {
    ...
    remove(inactiveWindow?: WindowType) {
    ...
    getWindowBySequence(targetSequence: WindowType['sequence']) {
      return windows.find(
...
const getZIndexFromElement = (element: Element) => {
  const zIndex = window
    .getComputedStyle(element)
    .getPropertyValue('z-index')
    .trim();

  if (!zIndex) return 700; // window 초기 z-index 값
...
const getActiveWindowsOrderByZIndexAsc = () => {
  ...
  const compareByZIndex = (
    window1: WindowType & { zIndex: number },
    window2: WindowType & { zIndex: number }
  ) => {
    if (window1.zIndex > window2.zIndex) return 1;
    if (window1.zIndex < window2.zIndex) return -1;

    if ((window1.sequence ?? 0) > (window2.sequence ?? 0)) return 1;
    return -1;
  };
...
const useEscCloseAndFocusable = (
  props: Props,
  {
    closeWin,
    windowRef,
  }: Pick<ReturnType<typeof useModel>, 'closeWin' | 'windowRef'>
) => {
  ...
  const removeInactiveWindow = (inactiveWindow?: WindowType) => {
  ...
  const keydownEsc = (event: KeyboardEvent) => {
    ...
    const activeWindowsSorted =
      getActiveWindowsOrderByZIndexAsc() as WindowType[];
  ...
  const setWindowActive = () => {
    ...
    if (windowRef.value?.dataset.sequence)
      windowRef.value.dataset.sequence = sequence?.toString();
  ...
  const setZIndexToWindow = ({
    ...
    if (props.isModal) {
      ...
      if (dimLayerElem) dimLayerElem.style.zIndex = zIndex.toString();
    }
    if (elem?.style) elem.style.zIndex = zIndex.toString();
  };
