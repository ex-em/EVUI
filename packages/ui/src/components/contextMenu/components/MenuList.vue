  ...
  props: {
    ...
    items: {
      ...
      validator: (list) => {
        if (
          list.some(
            (v) => v.children !== undefined && !Array.isArray(v.children)
          )
        ) {
          console.warn(
            "[EVUI][ContextMenu] children attribute must be 'Array' type."
          );
          return false;
        } else if (
          list.some(
            (v) => v.click !== undefined && typeof v.click !== 'function'
          )
        ) {
          console.warn(
            "[EVUI][ContextMenu] click attribute must be 'Function' type."
          );
          return false;
        }
        return true;
