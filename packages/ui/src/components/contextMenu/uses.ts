...
export const usePosition = () => {
  ...
  const rootMenuList = ref<InstanceType<typeof MenuList>>(null);
  const menuStyle: {
    top: string | null;
    left: string | null;
    right: string | null;
  ...
  const show = async (e: MouseEvent) => {
    ...
    await nextTick();
    const menuListRect =
      rootMenuList.value?.$el?.children[0].getBoundingClientRect();
    if (menuListRect) {
...
export const useMenuList = () => {
   ...
   * @returns null
   */
  const showChild = async (e: MouseEvent, children) => {
    ...
    const childMenuRect =
      childMenu.value?.$el?.children[0].getBoundingClientRect();
    ...
    if (docHeight < e.target.offsetTop + menuListHeight) {
      ...
      if (
        docWidth <
        targetUlX + targetUlWidth + menuListWidth + RIGHT_BUFFER_PX
      ) {
