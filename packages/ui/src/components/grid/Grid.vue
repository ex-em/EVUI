      ...
      <template #toolbarWrapper>
        ...
        <div class="filtering" :style="{ width: `${filteringItemsWidth}px` }">
          ...
          <div
            v-if="
              !isShowColumnFilteringItems &&
              Object.keys(hiddenFilteringItemsByColumn).length
            "
                ...
                >
                  <td
                    ...
                    :style="{
                      ...
                      'min-width': `${isRenderer(column) ? rendererMinWidth : minWidth}px`,
                      'border-right':
                        orderedColumns.length - 1 === cellIndex
                          ? 'none'
                          : '1px solid #CFCFCF',
                    }"
                    :disabled="row[ROW_DISABLED_INDEX]"
                  >
                    ...
                    <template v-else>
                      <div
                        :title="getConvertValue(column, row[2][column.index])"
                      >
...
export default {
  ...
  setup(props) {
    ...
    const stores = reactive({
      ...
      filterStore: [],
      store: computed(() => {
        const store = filterInfo.isFiltering
          ? stores.filterStore
          : stores.originStore;
    ...
    const pageInfo = reactive({
      ...
      currentPage: 0,
      pageTotal: computed(() =>
        props.option.page?.useClient
          ? stores.store.length
          : props.option.page?.total
      ),
    ...
    const resizeInfo = reactive({
      ...
      rowHeight: computed(() =>
        props.option.rowHeight > rowMinHeight
          ? props.option.rowHeight
          : rowMinHeight
      ),
      gridWidth: computed(() =>
    ...
    const { onSort, setSort } = sortEvent({
      sortInfo,
      stores,
      updatePagingInfo,
    });
    ...
    onUpdated(() => {
      filteringItemsWidth.value =
        elementInfo['grid-wrapper']?.offsetWidth / 1.5 || 0;
    });
    ...
    watch(
      ...
      () => {
        if (stores.store.length) {
          stores.store.forEach((row) => {
            row[ROW_DISABLED_INDEX] = props.disabledRows.includes(
              row[ROW_DATA_INDEX]
            );
          });
        }
    ...
    watch(
      ...
      async (index) => {
        ...
        if (index >= 0) {
          ...
          elementInfo.body.scrollTop =
            resizeInfo.rowHeight * highlightIdx.value;
        }
      }
    );
    watch(
    ...
    const setColumnFilteringItems = async (isInit) => {
      ...
      const conditionItems = filteringItemsRef.value?.getElementsByClassName(
        'filtering-items__item'
      );
      ...
      if (conditionItems) {
        for (let i = 0; i < conditionItems.length; i++) {
          ...
          const boxWidth =
            filteringItemsRef.value.getBoundingClientRect()?.width;
