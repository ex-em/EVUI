...
export default class EvChartBrush {
  ...
  init(isResize) {
    ...
    if (this.evChartBrushRef?.value) {
      const existedBrushCanvas =
        this.evChartBrushRef.value.querySelector('.brush-canvas');
  ...
  setBrushXAndWidth(canvasPosInfo, offsetX = 0, isDebounce, mode) {
    const { brushButtonWidth, brushCanvasWidth, axesXInterval, pixelRatio } =
      canvasPosInfo;
  ...
  removeBrushCanvas() {
    if (this.evChartBrushContainer) {
      const brushCanvas =
        this.evChartBrushContainer.querySelector('.brush-canvas');
  ...
  updateBrushIdxUseXPos() {
    ...
    const calculateBrushIdxByPos = () => {
      ...
      if (this.brushPosInfo) {
        const { brushRectX, brushRectWidth, brushCanvasWidth, axesXInterval } =
          this.brushPosInfo;
