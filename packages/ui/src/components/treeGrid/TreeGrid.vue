...
import Toolbar from './components/TreeGridToolbar.vue';
import GridPagination from '../grid/components/GridPagination.vue';
import GridSummary from '../grid/components/GridSummary.vue';
import ColumnSetting from '../grid/components/GridColumnSetting.vue';
import GridOptionButton from '../grid/icon/IconOptionButton.vue';
import {
  commonFunctions,
  scrollEvent,
  resizeEvent,
...
export default {
  ...
  setup(props) {
    ...
    const useGridSetting = computed(
      () => props.option?.useGridSetting?.use || false
    );
    ...
    const stores = reactive({
      ...
      filterStore: [],
      pagingStore: [],
      showTreeStore: computed(() =>
        stores.treeStore.filter((item) => item.show)
      ),
      searchStore: computed(() =>
        stores.treeStore.filter((item) => item.isFilter)
      ),
      store: computed(() =>
        filterInfo.isSearch ? stores.searchStore : stores.treeStore
      ),
      treeRows: props.rows,
      filteredColumns: [],
      ...
      orderedColumns: computed(() =>
        stores.filteredColumns.length
          ? stores.filteredColumns
          : stores.originColumns
      ),
      updatedColumns: computed(() => {
        const orderedColumnsIndexes = stores.orderedColumns?.map(
          (column) => column.index
        );
    ...
    const resizeInfo = reactive({
      ...
      gridWidth: computed(() =>
        props.width ? setPixelUnit(props.width) : '100%'
      ),
      gridHeight: computed(() =>
        props.height ? setPixelUnit(props.height) : '100%'
      ),
      isResize: false,
    });
    ...
    const { onRowClick, onRowDblClick } = clickEvent(selectInfo);
    ...
    const { setTreeNodeStore, handleExpand } = treeEvent({ stores, onResize });

    const { onSearch } = filterEvent({
      stores,
      filterInfo,
      pageInfo,
      getConvertValue,
    ...
    watch(
      ...
      (newColumns, prevColumns) => {
        const isSameColumns = () => {
          ...
          return prevColumnsFields.every((field) =>
            newColumnsFields.includes(field)
          );
        };
    ...
    watch(
      ...
      async (index) => {
        ...
        if (index >= 0) {
          const highlightNode = stores.store.find(
          ...
          if (pageInfo.usePage && !pageInfo.isInfinite) {
            ...
            pageInfo.highlightPage =
              highlightIndex === pageInfo.perPage ? page + 1 : page || 1;
            // 페이지 이동
    ...
    const getColumnStyle = (column, index) => {
      ...
      return {
        ...
        'min-width': render
          ? `${resizeInfo.rendererMinWidth}px;`
          : `${resizeInfo.minWidth}px`,
        'margin-right':
