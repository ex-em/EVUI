...
export const useDropdown = (
  ...
  const dropbox = ref<HTMLDivElement | null>(null);
   ...
   */
  const changeFilterText = (e: Event) => {
  ...
  const changeDropboxPosition = async () => {
    await nextTick();
    const selectHeight =
      selectWrapper.value?.getBoundingClientRect().height ?? 0;
    const selectY = selectWrapper.value?.getBoundingClientRect().y ?? 0;
    const dropboxHeight = dropbox.value?.getBoundingClientRect().height ?? 0;
