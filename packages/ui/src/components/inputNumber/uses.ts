...
import type { Emits, HandleType, ModelValue, Props, ValueParam } from './type';

export const getPrecision = (v: number) => {
 ...
 * */
export function getValueCloseToStep(
  val: number,
  ...
  if (maxPrecision) {
    // 소수점 값일 경우
    let multipleValue = +parseFloat((step * quotient).toString()).toFixed(
      maxPrecision
    );
    ...
    if (result > max) {
      multipleValue = +parseFloat((step * maxQuotient).toString()).toFixed(
        maxPrecision
      );
      result = +(multipleValue + min).toFixed(maxPrecision);
    } else if (result < min) {
...
export function useModel(props: Props, emit: Emits) {
   ...
   * */
  const getPrecisionValue = (val: ModelValue) =>
    props.precision && (val || val === 0)
      ? Number(val).toFixed(props.precision)
      : val;
   ...
   * */
  const validateValue = (val: ModelValue) => {
    let result = val;
    ...
    } else if ((props.min || props.min === 0) && val < props.min) {
      // 최소값보다 작을 경우
      result = props.min;
    } else if ((props.max || props.max === 0) && val > props.max) {
      // 최대값보다 클 경우
      result = props.max;
    } else {
      result = +val;
...
export function useStep(props: Props, params: ValueParam) {
   ...
   * @param type - 'up', 'down'
   * */
  const stepValue = (type: HandleType) => {
    ...
    if (!currentValue.value && currentValue.value !== 0) {
      ...
      const maxPrecision = Math.max(
        getPrecision(newValue),
        getPrecision(props.step)
      );
      const squaredValue = 10 ** maxPrecision;
      result =
        (Math.round(newValue * squaredValue) +
          Math.round(convertedStep * squaredValue)) /
        squaredValue;
    }
...
export const useInit = (
  props: Props,
  { currentValue, validateValue }: ValueParam
) => {
  ...
  const validateProps = () => {
    if (hasMaxProps && hasMinProps) {
      if (props.max <= props.min) {
        console.warn(
          '[EVUI][InputNumber] Max value must be greater than min value.'
        );
      }
    }
  ...
  const initValue = () => {
    ...
    if ((modelValue || modelValue === 0) && typeof modelValue === 'number') {
