...
export const useModel = () => {
  ...
  const isRangeMode = ['dateTimeRange', 'dateRange', 'dateMulti'].includes(
    props.mode
  );
  ...
  if (['dateTimeRange', 'dateTime'].includes(props.mode) && timeFormat) {
    if (
      ...
      currentValue = ref(
        getChangedValueByTimeFormat(timeFormat, props.modelValue)
      );
    } else {
      currentValue = ref(props.modelValue);
    }
  ...
  const validateValue = (curr) => {
    const dateRule = (targetDate) =>
      !!(targetDate.length === 10 && dateReg.exec(targetDate));
    ...
    if (props.mode === 'date') {
      ...
      isValid = curr.every(
        (value) => dateTimeRule(value) && checkValidDate(value)
      );
    }
...
export const useShortcuts = (param) => {
  ...
  const activeShortcut = (targetKey) => {
    const targetShortcut = usedShortcuts.find(
      (shortcut) => shortcut.key === targetKey
    );
    if (targetShortcut) {
  ...
  const formatDateTime = (targetDateTime) => {
    const dateTimeValue = targetDateTime
    ...
    if (isRange) {
      ...
      if (props.mode === 'dateTimeRange' && timeFormat?.length) {
        ...
        targetShortcut = usedShortcuts.find(({ shortcutDate }) => {
          const [sFromDate, sToDate] = shortcutDate();
          const formatFunc =
            props.mode === 'dateTimeRange' ? formatDateTime : formatDate;
          const isCorrectFromDate =
            formatFunc(sFromDate) === formatFunc(fromDate);
      ...
      const formatFunc =
        props.mode === 'dateTime' ? formatDateTime : formatDate;
