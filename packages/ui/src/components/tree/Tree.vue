...
export default {
  ...
  setup(props, { emit }) {
    ...
    function dblClickContent(nodeKey) {
      ...
      emit('dblclick-node', {
        title: dbClickedContent.title,
        value: dbClickedContent.value,
      });
    }
    ...
    function filterNode(value) {
      ...
      const filteredNodes = allNodeInfo.filter((nodeObj) =>
        isIncluded(nodeObj.node.title, value)
      );
