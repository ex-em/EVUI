...
export const useModel = (injectGroupSelectedLabel) => {
  ...
  const getNormalizedOptions = (options) => {
    ...
    if (
      (options.type === 'scatter' || options.type === 'heatMap') &&
      !options?.tooltip
    ) {
  ...
  const selectLabelInfo = cloneDeep(
    props.selectedLabel ?? injectGroupSelectedLabel?.value
  );
  ...
  const eventListeners = {
    click: async (e) => {
      await nextTick();
      const { seriesId, dataIndex, eventTarget, targetAxis } =
        e?.selected ?? {};
...
export const useZoomModel = (
  ...
  const createEvChartZoom = () => {
    if (evChartGroupRef?.value) {
      evChartInfo.dom = evChartGroupRef.value.querySelectorAll(
        '.ev-chart-container'
      );
      ...
      evChartInfo.dom = [
        evChartWrapper.value.querySelector('.ev-chart-container'),
      ];
  ...
  const setDataForUseZoom = (newData) => {
    if (isUpdateDataForUseZoom.value) {
      if (!isExecuteZoom.value) {
        evChartClone.data = evChartGroupRef
          ? cloneDeep(newData)
          : [cloneDeep(newData)];
      ...
      if (selectedLabelOrItem?.value) {
        if (typeof selectedLabelOrItem.value.dataIndex === 'number') {
          if (curBrushStartIdx >= (prevBrushStartIdx ?? 0)) {
            selectedLabelOrItem.value.dataIndex -=
          ...
          for (
            let idx = 0;
            idx < selectedLabelOrItem.value.dataIndex.length;
            idx++
          ) {
            if (curBrushStartIdx >= (prevBrushStartIdx ?? 0)) {
